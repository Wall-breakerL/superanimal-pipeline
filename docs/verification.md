...
结论：独立仓库的配置、清洗、导出、续跑与技术验收功能已实现，并通过历史数据回归。2026-09-10 23:14 GPU 恢复后的补验已通过：200 帧真实视频使用既有权重重新推理，随后清洗、导出和续跑验证均成功。详见 [GPU 实测](gpu-validation-20260910.md)。下方无 GPU 的失败记录保留为历史，不代表当前状态。
...
早期服务器现场检查：`nvidia-smi` 无设备，`torch.cuda.is_available()` 为 false；容器 cgroup 内存上限 2,147,483,648 字节（2 GiB），不能采用宿主机 `free` 显示的总内存作本容器配额。23:13 复查已恢复 RTX 4090、120 GiB 容器内存，补验记录独立保存。
...
受限状态下未继续反复重试。恢复 GPU 和充足内存后，已用 **新的输出目录**完成短视频 smoke：`check → run → verify`；没有覆盖早期失败 attempt 或原始实验结果。

`sap adapt` 的 4+4 epoch 参数有接口测试；本次没有重新训练，也没有对新适配效果作验证。zero-shot 模型下载和陌生服务器从零安装未完成现场验证。GitHub Actions 的在线结果以私有仓库 Actions 页面和实际运行记录为准，不能只凭配置文件认定通过。
